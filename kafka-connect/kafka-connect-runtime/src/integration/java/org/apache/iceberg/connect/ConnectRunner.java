...
final class ConnectRunner {
    ...
    public ConnectRunner(final int offsetFlushIntervalMs, Path pluginDir) {
    ...
    void startConnectCluster(final String connectorName, final int localPort, final int containerPort) {
        ...
        brokerProperties.put("advertised.listeners", "PLAINTEXT://localhost:" + containerPort +",EXTERNAL://localhost:9092");
        ...
        brokerProperties.put("inter.broker.listener.name", "PLAINTEXT");

//                + ",TESTCONTAINERS://host.testcontainers.internal:" + containerPort);
        brokerProperties.put("listeners",
                "PLAINTEXT://localhost:" + containerPort + ",EXTERNAL://localhost:9094" +
                        ",CONTROLLER://localhost:" + 9093
        );
        brokerProperties.put("listener.security.protocol.map", "PLAINTEXT:PLAINTEXT,CONTROLLER:PLAINTEXT,EXTERNAL:PLAINTEXT"
//                ",TESTCONTAINERS:PLAINTEXT"
        );
    ...
    private Map<String, String> getWorkerProperties() {
        ...
        workerProps.put("plugin.path", pluginDir.toAbsolutePath().toString());
